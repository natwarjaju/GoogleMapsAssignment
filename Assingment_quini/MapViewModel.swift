...
class MapViewModel:NSObject, ObservableObject, CLLocationManagerDelegate  {
    ...
    @Published var shouldShowNoResultsFound = false
    ...
    func searchLocations() {
        self.shouldShowNoResultsFound = false
        ...
        MKLocalSearch(request: request).start(completionHandler: {(response, error) in
            ...
            guard let results = response else {
                self.shouldShowNoResultsFound = true
                return
            }
    ...
    func drawPinAndPathForSelectionLocation(location: CLLocationCoordinate2D,
                                            name: String,
                                            isFromHistoryView: Bool = false) {
        ...
        directions.calculate { (directionsResponse, error) in
            ...
            if let fastestRoute = directionsResponse?.routes.first?.polyline {
                ...
                self.mapView.setVisibleMapRect(fastestRoute.boundingMapRect,
        ...
        if (!isFromHistoryView) {
            if (!dataManager.getSavedLocations().isEmpty) {
                let previousSearchEntry = dataManager.getSavedLocations().first
                if previousSearchEntry?.name == name {
                locations.removeAll()
